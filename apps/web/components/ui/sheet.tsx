  ...
  "fixed z-50 gap-4 bg-white shadow-lg transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500 data-[state=open]:animate-in data-[state=closed]:animate-out",
  {
    variants: {
      side: {
        ...
        bottom:
          "inset-x-0 bottom-[var(--keyboard-inset-bottom,0px)] border-t pb-[var(--sheet-bottom-inset)] data-[state=closed]:slide-out-to-bottom data-[state=open]:slide-in-from-bottom",
  ...
  (
    ...
    <SheetPortal>
      ...
      >
        {children}
        {showClose && (
          <SheetPrimitive.Close className="absolute top-[calc(var(--safe-area-top)+1rem)] right-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:ring-2 focus:ring-ring focus:outline-none disabled:pointer-events-none">
            <X className="size-4" />
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Title
    ref={ref}
    className={cn("text-lg leading-none font-semibold tracking-tight text-charcoal", className)}
    {...props}
  />
));
